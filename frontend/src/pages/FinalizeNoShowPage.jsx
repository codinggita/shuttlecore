...
const FinalizeNoShowPage = () => {
  ...
  const handleFinalize = () => {
    setIsFinalized(true);
    showToast("No-Show Finalized", "Marcus Thorne logged as no-show. Route recalculated for SV-402.", "error");
  };
            ...
            <motion.div variants={itemVariants} className="mb-10">
              ...
              <div className="flex items-center gap-4 mb-2">
                <div className="w-12 h-12 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center">
                  <span className="material-symbols-outlined text-rose-500 text-2xl">do_not_disturb_on</span>
                </div>
                <h1 className="text-3xl md:text-5xl font-black text-main tracking-tighter">Finalize No-Show</h1>
              </div>
              <p className="text-muted text-sm md:text-base font-medium">Confirm the no-show incident and trigger downstream logistics recalculation.</p>
            </motion.div>

            {!isFinalized ? (
              <>
                {/* Rider Summary */}
                <motion.div variants={itemVariants} className="dashboard-card !p-8 border border-rose-500/20 shadow-[0_0_30px_rgba(239,68,68,0.05)] mb-6">
                  <h3 className="text-sm font-black uppercase tracking-widest text-muted mb-6">Incident Summary</h3>
                  <div className="flex items-center gap-5 mb-8 pb-8 border-b border-[var(--border)]">
                    <div className="w-16 h-16 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center text-2xl font-black text-rose-400">MT</div>
                    <div>
                      <p className="text-xl font-black text-main">Marcus Thorne</p>
                      <p className="text-sm text-muted font-bold">Booking #TX-9012 · VIP Tier</p>
                      <p className="text-xs text-rose-400 font-black uppercase tracking-widest mt-1">No-Show Detected</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-6">
                    {[
                      { label: "Pickup Point", value: "North Financial Plaza" },
                      { label: "Scheduled Time", value: "14:00 UTC" },
                      { label: "Wait Duration", value: "12 mins 30s" },
                    ].map((item, i) => (
                      <div key={i}>
                ...
                <motion.div variants={itemVariants} className="dashboard-card !p-8 border border-[var(--border)] mb-8">
                  <h3 className="text-sm font-black uppercase tracking-widest text-muted mb-6">Downstream Actions on Confirm</h3>
                  <div className="space-y-4">
                    {[
                      { icon: "route", label: "Route SV-402 recalculated to next booking", color: "text-[var(--primary)]" },
                      { icon: "assignment_late", label: "Auto-generate incident log for post-op review", color: "text-amber-400" },
                      { icon: "notifications", label: "Notify Sector 7 operations of schedule gap", color: "text-rose-400" },
                      { icon: "credit_card", label: "Void hold charge for booking #TX-9012", color: "text-emerald-400" },
                    ].map((action, i) => (
                      <div key={i} className="flex items-center gap-4 p-4 bg-[var(--surface-muted)] rounded-xl border border-[var(--border)]">
                        ...
                        <p className="text-sm font-bold text-main">{action.label}</p>
                      </div>
                    ))}
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="flex gap-4">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={handleFinalize} className="flex-1 py-4 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-2xl shadow-rose-500/20 flex items-center justify-center gap-3">
                    <span className="material-symbols-outlined">do_not_disturb_on</span> Confirm No-Show
                  </motion.button>
                </motion.div>
              </>
            ) : (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="dashboard-card !p-12 border border-rose-500/20 flex flex-col items-center text-center">
                <div className="w-24 h-24 bg-rose-500/10 rounded-full flex items-center justify-center mb-6 border border-rose-500/20">
                  <span className="material-symbols-outlined text-5xl text-rose-500">task_alt</span>
                </div>
                <h2 className="text-2xl font-black text-main mb-2">No-Show Logged</h2>
                <p className="text-muted text-sm max-w-md mb-8">Marcus Thorne has been flagged. SV-402 is being rerouted. Incident report queued for review.</p>
...
export default FinalizeNoShowPage;
